...
device = HPMA115S0.HPMA115S0("/dev/serial0")
device.init()
device.startParticleMeasurement()
device.readParticleMeasurement()
...
def update(frame):
    t = datetime.now()
    x_data.append(t)
    if (device.readParticleMeasurement()):
        print("PM2.5 %s:%d ug/m3" %(str(t),device._pm2_5))
        print("PM10 %s:%d ug/m3" %(str(t),device._pm10))
	f.write("%s,%d,%d\n" %(t, device._pm2_5, device._pm10))

    pm2_data.append(device._pm2_5) 
    pm2_max.append("20") 
    pm10_data.append(device._pm10) 
